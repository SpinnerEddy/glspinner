...
glspinnerには`glspinner-linter`スキルにより`eslint.config.mjs`（ESLint、`typescript-eslint`ベース）と`.prettierrc.json`/`.prettierignore`（Prettier）が導入済みで、`package.json`にも`lint`/`lint:fix`/`format`/`format:check`の4スクリプトがある。ただしESLintが機械的に強制しているのはクラス/インターフェース/型エイリアスのPascalCase、メソッドのcamelCase、private/protectedメンバーのcamelCase（アンダースコア接頭辞禁止）という命名規則の一部のみで、`==`/`!=`と`===`/`!==`の混在は下記「未解決・揺れがある事項」1番のとおり「厳密な規約ではない」ドキュメント済みの揺れのため`eqeqeq`ルールを意図的に無効化している（`eslint.config.mjs`内にコメントで明記）。`Constants`接尾辞の有無、getter/setter方式の使い分け、各クラスファミリー固有の慣習など、命名・フォーマット規約の大半は依然としてESLint/Prettierではカバーされておらず、「規約」は既存コードの書き方そのものが実質的な一次情報源であることに変わりはない。
...
- 2026-07-19（追記4）: `glspinner-linter`スキルの実行により`eslint.config.mjs`/`.prettierrc.json`/`.prettierignore`と`lint`/`lint:fix`/`format`/`format:check`スクリプトが実際に導入されたため、「この文書について」節冒頭の「ESLint/Prettierは存在しない」という記述が陳腐化していた（`CLAUDE.md`側も同様に陳腐化しており、`glspinner-document`スキルで別途修正済み）。ESLintが命名規則の一部（クラス/インターフェース/型エイリアスのPascalCase、メソッド・private/protectedメンバーのcamelCase）と`eqeqeq`の意図的な無効化のみを機械的にカバーしており、規約の大半は引き続き`.claude/rules/`が一次情報源であることを明記する形に更新した。
