...
export class RecordGuiController {
    ...
    static initialize(onRecordStart: () => void, onRecordEnd: () => void, onChangeClockType: (clockType: ClockType) => void) {
        ...
        GuiUtility.addElement({ fps: 60 }, 'fps', (value: number) => {
            this.fps = value;
            this.onChangeClockType?.(this.clockType);
        });
        GuiUtility.addElement({ fixedFrameInterval: 60 }, 'fixedFrameInterval', (value: number) => {
            this.fixedFrameInterval = value;
            this.onChangeClockType?.(this.clockType);
        });
        GuiUtility.addElement({ frameNum: 300 }, 'frameNum', (value: number) => {
            this.frameNum = value;
        });
        GuiUtility.addElement({ saveName: 'test' }, 'saveName', (value: string) => {
            this.saveName = value;
        });
        GuiUtility.addFolder('Resolution');
        GuiUtility.addElement({ width: 800 }, 'width', (value: number) => {
            this.width = value;
        });
        GuiUtility.addElement({ height: 800 }, 'height', (value: number) => {
            this.height = value;
        });
