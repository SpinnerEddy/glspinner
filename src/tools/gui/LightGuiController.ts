...
export class LightGuiController {
    ...
    static initialize() {
        ...
        GuiUtility.addColorElement({ ambientColor: '#00000000' }, 'ambientColor', (value: string) => {
            this.ambientColor = value;
        });

        GuiUtility.addFolder('LightDirection');
        GuiUtility.addElementWithRange({ lightDirectionX: -0.5 }, 'lightDirectionX', -1.0, 1.0, (value: number) => {
            this.lightDirectionX = value;
        });
        GuiUtility.addElementWithRange({ lightDirectionY: 0.5 }, 'lightDirectionY', -1.0, 1.0, (value: number) => {
            this.lightDirectionY = value;
        });
        GuiUtility.addElementWithRange({ lightDirectionZ: 0.5 }, 'lightDirectionZ', -1.0, 1.0, (value: number) => {
            this.lightDirectionZ = value;
        });
        GuiUtility.resetFolder();

        GuiUtility.addFolder('EyeDirection');
        GuiUtility.addElementWithRange({ eyeDirectionX: 0.0 }, 'eyeDirectionX', 0.0, 20.0, (value: number) => {
            this.eyeDirectionX = value;
        });
        GuiUtility.addElementWithRange({ eyeDirectionY: 0.0 }, 'eyeDirectionY', 0.0, 20.0, (value: number) => {
            this.eyeDirectionY = value;
        });
        GuiUtility.addElementWithRange({ eyeDirectionZ: 20.0 }, 'eyeDirectionZ', 0.0, 20.0, (value: number) => {
