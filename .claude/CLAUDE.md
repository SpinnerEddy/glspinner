...
以下の**両方**を満たす場合に限り、Claude Codeがsrc/配下（シェーダー含む）を直接編集してよい。当初は全面禁止だったが、「設計判断は会話で固まっているのに、確定した内容をユーザーが手で書き写すだけの往復」でテンポが悪化する問題が実際に発生したため、目的（考える部分は自分でやる）を保ったまま往復コストだけを削る形で緩和した。`examples/`配下（`sample.ts`等の開発用サンドボックス、公開ライブラリには含まれない）も同じ例外条件で扱う——`examples/`は元々「常時編集可」リストにも入っていなかったが、src/と同じ条件（内容が完全に確定していて、かつユーザーが明示的に同意している）を満たせば直接編集してよい。
