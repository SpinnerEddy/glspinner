...
export class GeometryBuffer extends BaseBuffer {
    ...
    constructor(gl: WebGL2RenderingContext, vertices: Float32Array, color: Float32Array, normal: Float32Array, uv: Float32Array = new Float32Array()) {
        super(gl);
    ...
    createInterleavedArray(vertices: Float32Array, color: Float32Array, normal: Float32Array, uv: Float32Array): Float32Array {
        ...
        for (let i = 0; i < vertexNum; i++) {
            ...
            interleavedArray.set(vertices.subarray(vertexOffset, vertexOffset + AttributeElementSize.aPosition), arrayIndex);
            arrayIndex += AttributeElementSize.aPosition;

            const colorOffset = i * AttributeElementSize.aColor;
            interleavedArray.set(color.subarray(colorOffset, colorOffset + AttributeElementSize.aColor), arrayIndex);
            ...
            if (normal.length > 0) {
                ...
                interleavedArray.set(normal.subarray(normalOffset, normalOffset + AttributeElementSize.aNormal), arrayIndex);
                arrayIndex += AttributeElementSize.aNormal;
            }

            if (uv.length > 0) {
                const uvOffset = i * AttributeElementSize.aUv;
                interleavedArray.set(uv.subarray(uvOffset, uvOffset + AttributeElementSize.aUv), arrayIndex);
